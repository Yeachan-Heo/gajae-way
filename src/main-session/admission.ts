...
async function dispatchAdmittedOperation(
	...
	// A scheduler admission carries no surface id; the durable attribution is
	// derived from the canonical request's `origin` instead.
...
/** Durable attribution for one admission: an operator surface, or the scheduler. */
...
/** Typed input for the in-process scheduler entry point. */
...
export function createMainAdmissionHandler(
	...
	const assertMutationReadiness = (): void => {
	...
	// Shared durable pipeline for every admission, regardless of origin: one
	// pre-effect claim, one broker dispatch, one finalization. Both entry points
	// funnel through here so there is exactly one writer to the main session.
	const admit = async (
		idempotencyKey: string,
		text: string,
		requestJson: string,
		...
		try {
			...
			abandonDefinitivelyRejectedClaim(target, idempotency, idempotencyKey, requestJson, intentJson, error);
			throw error;
		}
	...
	/**
	 * The only entry point bound to the `main.submit` RPC method. Its parameter
	 * allowlist is unchanged, so no RPC caller can reach the system-event path.
	 */
	const submitFromRpc = async (params: unknown): Promise<MainAdmissionResponse> => {
		const request = parseRequest(params);
		assertMutationReadiness();
		...
		const deliveredAs: DeliveredAs = ownerSurfaceIds.has(surface.id)
			? target.turnState === "idle"
				...
				: "steer"
			: "follow_up";
		return await admit(request.idempotencyKey, request.text, requestJson, deliveredAs, {
			kind: "surface",
	...
	/**
	 * In-process entry point for the scheduler. It is deliberately NOT reachable
	 * from `main.submit`: it takes typed arguments rather than `unknown` params,
	 * it is never bound to an RPC method, and `parseRequest` would reject its
	 * shape anyway. Delivery is pre-classified as `follow_up`, so a scheduled
	 * system event always enqueues behind the operator turn instead of steering
	 * it, and it never consults `target.turnState`.
	 */
	const admitSystemEvent = async (input: SystemEventAdmission): Promise<MainAdmissionResponse> => {
		...
		assertMutationReadiness();
		// No surface_id: the scheduler is attributed by origin, so it cannot
		// borrow admission authority from a configured surface.
		const requestJson = canonicalJson({
			idempotency_key: idempotencyKey,
			origin: SCHEDULER_ORIGIN,
			text,
		});
		return await admit(idempotencyKey, text, requestJson, "follow_up", { kind: "scheduler" });
	};
