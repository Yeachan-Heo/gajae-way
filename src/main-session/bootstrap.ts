...
import { type HostSupervisor, HostSupervisorError, type SupervisorVerification } from "./supervisor";
...
function requireIntent(state: BootstrapState, intent: BootstrapIntent | undefined): BootstrapIntent {
	if (!intent)
		throw new BootstrapError(
			...
			`${state} bootstrap state has no durable external-session intent.`,
		);
	return intent;
}
...
function checkedVerification(
	...
	if (identity.sessionId !== sessionId) {
		throw new BootstrapError(
			...
			"The broker did not return the requested external session id.",
		);
	}
	if (verified.transcriptProof === "pending") {
		if (identity.transcript || verified.transcriptEntries.length !== 0 || verified.initialRingCheckpoint) {
			throw new BootstrapError(
				...
				"A pending transcript proof carried fingerprint or ring-boundary evidence.",
			);
		}
	...
	if (verified.transcriptProof !== "proven" || !identity.transcript || !verified.initialRingCheckpoint) {
		throw new BootstrapError(
			"transcript_proof_invalid",
			"A proven transcript proof requires a complete transcript and its ring boundary.",
		);
	}
	...
	if (entries.some((entry) => !entry.id.trim())) {
		throw new BootstrapError(
			...
			"The broker transcript proof contains an entry without a stable id.",
		);
	}
	...
	if (observed.entryCount !== identity.transcript.entryCount || observed.sha256 !== identity.transcript.sha256) {
		throw new BootstrapError(
			...
			"The broker transcript fingerprint did not match its complete transcript snapshot.",
		);
	}
	return {
		identity,
...
export async function bootstrapMainSession(options: BootstrapOptions): Promise<BootstrapSuccess> {
	...
	if (!options.sessionId.trim())
		throw new BootstrapError("session_id_required", "gajaeway bootstrap requires an external session id.");
	...
	if (current.bootstrapState !== "ABSENT") {
		throw new BootstrapError(
			"bootstrap_state_not_absent",
			`Cannot bootstrap while state is ${current.bootstrapState}.`,
		);
	}
	...
	try {
		...
		options.state.commitBootstrap(
			"CREATED",
			intent,
			proof.identity,
			options.profile,
			proof.ringCheckpoint,
		...
		throw new BootstrapError("external_session_unavailable", error instanceof Error ? error.message : String(error), {
			cause: error,
		});
	}
}
...
export async function recoverBootstrap(
	...
	if (current.bootstrapState === "COMMITTED") {
		if (!current.mainIdentity)
			throw new BootstrapError("main_identity_missing", "COMMITTED state has no main identity.");
	...
	try {
		...
		return failClosed(
			options.state,
			error instanceof BootstrapError || error instanceof HostSupervisorError
				? error.reason
