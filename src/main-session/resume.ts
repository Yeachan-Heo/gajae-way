...
import { type HostSupervisor, HostSupervisorError, type SupervisorTailEvents } from "./supervisor";
...
export async function strictResumeMainSession(options: ResumeOptions): Promise<ResumedMainSession> {
	...
	if (durable.bootstrapState !== "COMMITTED" || !durable.mainIdentity) {
		return failClosed(
			...
			"The daemon cannot resume before an explicit committed bootstrap.",
		);
	}
	if (
		...
		return failClosed(
			options.state,
			"profile_drift",
			"The identity/security profile projection changed without approval.",
		);
	}
	if (options.profile.externalSessionId && options.profile.externalSessionId !== durable.mainIdentity.sessionId) {
		return failClosed(
			...
			"The profile selected a different external session than the durable adopted identity.",
		);
	}
	let verified;
	try {
	...
	if (!sameExternalSession(durableIdentity, current)) {
		return failClosed(
			...
			"The broker returned a different external session identity.",
		);
	}
	...
	if (durable.transcriptProof === "pending") {
		if (verified.transcriptProof === "proven") {
			if (
				...
				verified.transcriptEntries.some((entry) => !entry.id.trim())
			) {
				return failClosed(
					options.state,
					"transcript_proof_invalid",
					"The broker returned an invalid complete transcript proof or ring boundary.",
				);
			}
			const transcriptDeliveryProgress: TranscriptDeliveryProgress = {
				...(verified.transcriptEntries.at(-1) === undefined
					? {}
					: { lastEntryId: verified.transcriptEntries.at(-1)?.id }),
			...
			if (
				...
				return failClosed(
					...
					"The broker transcript fingerprint did not match its complete snapshot.",
				);
			}
			try {
				options.state.persistTranscriptProof(
					durableIdentity,
					current,
			...
			return failClosed(
				options.state,
				"transcript_proof_invalid",
				"The broker returned an invalid pending transcript proof.",
			);
		} else {
		...
		return failClosed(
			options.state,
			"transcript_proof_invalid",
			"The broker returned an invalid proof state for the durable transcript identity.",
		);
	}
	...
	if (durable.growthIntent) {
		if (!sameExternalSession(durable.growthIntent.base, current)) {
			return failClosed(
				...
				"The adopted session changed while a growth intent was open.",
			);
		}
		if (
			verificationState === "verified" &&
			!attestsExternalTranscriptGrowth(
				durable.growthIntent.base,
				verified.transcriptEntries.map((entry) => entry.payload),
			)
		) {
			return failClosed(
				options.state,
				"growth_intent_mismatch",
				"The broker transcript changed outside append-only growth recovery.",
			);
		}
		...
		return failClosed(
			options.state,
			"main_identity_mismatch",
			"The persisted external session identity no longer matches broker evidence.",
		);
	}
	try {
		...
		return failClosed(
			options.state,
			"profile_tunables_revision_failed",
			error instanceof Error ? error.message : String(error),
			error,
		);
	}
