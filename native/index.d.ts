...
export declare class WayCore {
   ...
   */
  memoryIndexRebuild(profileDigest: string, documents: Array<MemoryDocumentInput>, masks: Array<MemoryMaskInput>): number
  /**
   ...
   * disable it.
   */
  alertsEvaluateAdapterDisconnects(thresholdMs: number): Array<string>
  /** Currently raised alert conditions, for `way.status` and operator tooling. */
  alertsRaised(): Array<string>
  mainEventKinds(): Array<string>
  scheduleJobUpsert(input: ScheduleJobUpsertInput): ScheduleJobOutput
  scheduleJobGet(jobId: string): ScheduleJobOutput
  scheduleJobList(input?: ScheduleJobListInput | undefined | null): ScheduleJobListOutput
  scheduleJobDelete(jobId: string): boolean
  scheduleDueClaim(nowMs: number, runId: string): ScheduleDueClaimOutput
  scheduleRunFinalize(input: ScheduleRunFinalizeInput): ScheduleRunOutput
  scheduleRunList(jobId: string, cursor?: string | undefined | null, limit?: number | undefined | null): ScheduleRunListOutput
  scheduleInFlightRuns(): Array<ScheduleRunOutput>
  scheduleOverdueCollapse(input: ScheduleOverdueCollapseInput): ScheduleRunOutput | null
  scheduleSetNextFire(jobId: string, nextFireAtMs: number | undefined | null, nowMs: number): void
}
...
export interface MemoryMaskInput {
  sessionKind: string
 ...
 */
export interface ScheduleDueClaimOutput {
  claimed: boolean
  refusedFailedClosed: boolean
  reason?: string
  job?: ScheduleJobOutput
  run?: ScheduleRunOutput
}

export interface ScheduleJobListInput {
  state?: string
  cursor?: string
  limit?: number
}

export interface ScheduleJobListOutput {
  jobs: Array<ScheduleJobOutput>
  nextCursor?: string
}
...
export interface ScheduleRunListOutput {
  runs: Array<ScheduleRunOutput>
  nextCursor?: string
}
