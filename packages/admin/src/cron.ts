...
/**
 * The first local-time minute strictly after `from` that the schedule matches,
 * or null for an unparseable schedule or one that cannot fire (e.g. 31 February).
 * Bounded to a 366-day horizon so a pathological schedule cannot spin.
 */
export function nextCronFire(schedule: string, from: Date): Date | null {
	...
	while (cursor.getTime() <= horizon) {
		if (!cron.month.has(cursor.getMonth() + 1)) {
			cursor.setMonth(cursor.getMonth() + 1, 1);
			cursor.setHours(0, 0, 0, 0);
			continue;
		}
		const dayOfMonthHit = cron.dayOfMonth.has(cursor.getDate());
		const dayOfWeekHit = cron.dayOfWeek.has(cursor.getDay());
		const dayHit = cron.dayUnion ? dayOfMonthHit || dayOfWeekHit : dayOfMonthHit && dayOfWeekHit;
		if (!dayHit) {
