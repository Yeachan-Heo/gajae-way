...
test("authoring answers are accepted as bare JSON, fenced JSON, or prose around the array; garbage still fails", () => {
	...
	expect(parseAuthoredArray("Clear picture now. Both X and Threads are working.\n" + JSON.stringify(arr) + "\nDone.")).toEqual(arr);
	expect(parseAuthoredArray('note with [brackets] inside: [{"eventId":"e1","note":"see [ref] here"}]')).toEqual([{ eventId: "e1", note: "see [ref] here" }]);
