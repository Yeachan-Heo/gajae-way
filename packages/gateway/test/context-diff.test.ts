...
import { CONVERSATION_DIFF_MAX_AGE_MS, CONVERSATION_DIFF_MAX_ROWS, GatewayDatabase } from "../src/store/db";
...
test("287-row stale backlog selects only newest recent rows chronologically and never replays omissions", async () => {
	...
	for (let index = 0; index < 246; index++)
	...
	const first = db.contextPrepareWindow(ORIGIN_KEY, "trigger-current", NOW);
	...
	expect(first.diagnostics.expired).toBe(246);
	...
	const next = db.contextPrepareWindow(ORIGIN_KEY, "trigger-next", new Date(NOW.getTime() + 1_000));
	expect(next.rows).toEqual([]);
	expect(next.diagnostics.expired).toBe(246);
});
...
test("more than the count bound keeps newest N once and expires older recent rows", async () => {
	...
	const window = db.contextPrepareWindow(ORIGIN_KEY, "trigger", NOW);
	...
	expect(db.contextPrepareWindow(ORIGIN_KEY, "trigger-2", new Date(NOW.getTime() + 1_000)).rows).toEqual([]);
...
test("reset floor survives restart and excludes every pre-reset row", async () => {
	...
	const window = restarted.contextPrepareWindow(ORIGIN_KEY, "trigger", new Date(NOW.getTime() + 2_000));
