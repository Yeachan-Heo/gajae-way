...
describe("offline authority command with kernel-exclusive gateway ownership", () => {
	...
	test("legacy schema is backed up before migration 22 and authority adoption", async () => {
		...
		legacy.exec(
			"ALTER TABLE memory_intents DROP COLUMN quarantine_reason; ALTER TABLE memory_intents DROP COLUMN attempts; ALTER TABLE deliveries DROP COLUMN last_error",
		);
