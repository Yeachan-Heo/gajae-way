...
impl fmt::Display for SocketBootError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::StateDirectoryIsSymlink => {
                formatter.write_str("RPC state directory must not be a symlink")
            }
            Self::StateDirectoryNotDirectory => {
                formatter.write_str("RPC state directory is not a directory")
            }
            Self::StateDirectoryForeign { uid } => {
                write!(
                    formatter,
                    "RPC state directory is owned by uid {uid}, not this daemon"
                )
            }
            Self::SocketOutsideStateDirectory => {
                formatter.write_str("RPC socket must be directly inside the state directory")
            }
            Self::SocketParentIsSymlink => {
                formatter.write_str("RPC socket parent must not be a symlink")
            }
            Self::SocketPathIsSymlink => {
                formatter.write_str("RPC socket path must not be a symlink")
            }
            Self::ExistingPathNotSocket => {
                formatter.write_str("RPC socket path is occupied by a non-socket file")
            }
            Self::ExistingSocketForeign { uid } => {
                write!(
                    formatter,
                    "RPC socket is owned by uid {uid}, not this daemon"
                )
            }
            Self::ActiveSocket => {
                formatter.write_str("RPC socket is already accepting connections")
            }
        }
    }
}
...
pub fn bind_socket(state_dir: &Path, socket_path: &Path) -> Result<UnixListener, SocketBootError> {
    ...
    if state_metadata.uid() != uid {
        return Err(SocketBootError::StateDirectoryForeign {
            uid: state_metadata.uid(),
        });
    }

    let parent = socket_path
        .parent()
        .ok_or(SocketBootError::SocketOutsideStateDirectory)?;
    ...
    match fs::symlink_metadata(socket_path) {
        Ok(metadata) => {
            ...
            if metadata.uid() != uid {
                return Err(SocketBootError::ExistingSocketForeign {
                    uid: metadata.uid(),
                });
            }
...
mod tests {
    use std::{
        ...
        os::{
            fd::AsRawFd,
            unix::fs::{FileTypeExt, PermissionsExt, symlink},
        },
        path::PathBuf,
        sync::atomic::{AtomicU64, Ordering},
    };

    use super::{PeerAuth, SocketBootError, bind_socket, current_uid, peer_auth_for_uid, peer_uid};
    ...
    fn same_uid_peer_is_accepted_by_the_real_platform_credential_path() {
        ...
        assert_eq!(
            peer_auth_for_uid(current_uid(), peer_uid(server.as_raw_fd()).unwrap()),
            PeerAuth::Verified
        );
        drop(client);
        drop(server);
    ...
    fn foreign_file_and_symlink_are_refused_without_deletion() {
        ...
        assert!(matches!(
            bind_socket(&state, &socket),
            Err(SocketBootError::ExistingPathNotSocket)
        ));
        ...
        assert!(matches!(
            bind_socket(&state, &socket),
            Err(SocketBootError::SocketPathIsSymlink)
        ));
        assert!(
    ...
    fn active_same_uid_socket_is_never_replaced() {
        ...
        assert!(matches!(
            bind_socket(&state, &socket),
            Err(SocketBootError::ActiveSocket)
        ));
        assert!(
            fs::symlink_metadata(&socket)
                .unwrap()
    ...
    fn linux_second_os_user_is_rejected_by_real_so_peercred_when_enabled() {
        ...
        assert_eq!(
            unsafe { libc::geteuid() },
            0,
            "cross-UID peer test must run as root"
        );
        ...
        if child == 0 {
            unsafe {
                ...
                let status = if std::os::unix::net::UnixStream::connect(&socket).is_ok() {
                    0
                } else {
                    3
                };
