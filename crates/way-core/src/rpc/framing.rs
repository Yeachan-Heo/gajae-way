    ...
    pub async fn next_frame(&mut self) -> Result<Option<DecodedFrame>, FrameReadError> {
        loop {
            ...
            let read = self
                ...
                .read(&mut buffer)
                .await
                .map_err(|error| FrameReadError::Io(error.kind()))?;
            if read == 0 {
    ...
    async fn next_socket_frame(&mut self) -> Result<Option<DecodedFrame>, FrameReadError> {
        loop {
            ...
            self.reader
                ...
                .await
                .map_err(|error| FrameReadError::Io(error.kind()))?;
            let available = socket_available_bytes(self.reader.as_raw_fd())
                .map_err(|error| FrameReadError::Io(error.kind()))?;
            if available == 0 {
                let mut byte = [0_u8; 1];
...
pub fn decode_line(line: &[u8]) -> Result<DecodedFrame, FrameReadError> {
    ...
    match value {
        ...
        Value::Object(object) => parse_request(object)
            .map(DecodedFrame::Request)
            .map_err(FrameReadError::InvalidRequest),
        _ => Err(FrameReadError::InvalidRequest(RpcError::invalid_request(
...
fn parse_request(mut object: Map<String, Value>) -> Result<RpcRequest, RpcError> {
    ...
    if let Some(unexpected) = object.keys().next() {
        return Err(RpcError::invalid_request(format!(
            "unknown request member: {unexpected}"
        )));
    }
...
async fn handle_request(
    ...
    if method == "rpc.cancel" {
        ...
        return write_half
            .write_all(&result_response(id, json!({ "cancelled": cancelled })))
            .await
            .is_ok();
    }
    ...
    if in_flight.contains_key(&key) {
        return write_half
            .write_all(&error_response(
                id,
                RpcError::invalid_request("duplicate_id"),
            ))
            .await
            .is_ok();
    }
...
async fn shutdown_connection(
    ...
    for request in requests {
        if write_half
            .write_all(&error_response(
                request.id,
                RpcError::internal("shutting_down"),
            ))
            .await
            .is_err()
        {
...
mod tests {
    ...
    use super::{DecodedFrame, FrameReadError, MAX_FRAME_BYTES, NdjsonReader, decode_line};
    ...
    async fn blank_lines_are_ignored_and_one_object_is_decoded() {
        ...
        assert_eq!(
            ...
            Some(DecodedFrame::Blank)
        );
        assert!(
            matches!(reader.next_frame().await.unwrap(), Some(DecodedFrame::Request(request)) if request.method == "way.health")
        );
    }
