...
use crate::store::{Clock, Store, StoreError, SystemClock, meta_get_tx, meta_set_tx};
...
impl fmt::Display for JournalError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidPayload => {
                formatter.write_str("event payload_json must contain valid JSON")
            }
            ...
            Self::InvalidClaimTtl => {
                formatter.write_str("consumer claim TTL must be in 5000..=600000 ms")
            }
            Self::InvalidCursor => formatter.write_str("invalid journal cursor"),
            Self::CursorBeforeRetention(gap) => write!(
                formatter,
                "cursor is before retention floor; resync at {}",
                gap.resync_cursor
            ),
            Self::ConsumerClaimHeld => formatter.write_str("consumer claim is already held"),
            Self::ClaimNotHeld => {
                formatter.write_str("consumer claim is not held by this claim id")
            }
            Self::ClaimExpired => formatter.write_str("consumer claim has expired"),
            Self::CursorRegression => {
                formatter.write_str("consumer commit cursor regresses its checkpoint")
            }
            Self::CursorAheadOfHead => {
                formatter.write_str("consumer commit cursor is beyond journal head")
            }
...
impl EventJournal {
    ...
    pub fn read_filtered(
        ...
        if requested.journal_generation != generation {
            ...
            return Ok(JournalRead {
                events: Vec::new(),
                next_cursor: gap.resync_cursor,
                gap: Some(gap),
            });
        }
        if requested.seq < floor {
            let gap = JournalGap {
                missing_from: Cursor::new(
                    generation,
                    requested.seq.checked_add(1).ok_or(JournalError::Overflow)?,
                ),
                missing_to: Cursor::new(generation, floor),
                resync_cursor: Cursor::new(generation, floor),
            };
        ...
        let next_cursor =
            if events.len() < usize::try_from(limit).map_err(|_| JournalError::Overflow)? {
                Cursor::new(
                    generation,
                    head.try_into().map_err(|_| JournalError::Overflow)?,
                )
            } else {
                events
                    .last()
    ...
    pub fn claim_consumer(
        &self,
        consumer_id: &str,
        claim_ttl_ms: Option<u64>,
    ) -> JournalResult<ConsumerClaim> {
        if consumer_id.trim().is_empty() {
            return Err(JournalError::InvalidProof(
                "consumer_id must not be empty".to_owned(),
            ));
        }
        ...
        let now = self.clock.now_ms();
        let expires_at = now
            .checked_add(ttl_ms.try_into().map_err(|_| JournalError::Overflow)?)
    ...
    pub fn commit_consumer(
        ...
        let checkpoint_cursor =
            Cursor::from_str(&checkpoint.0).map_err(|_| JournalError::InvalidCursor)?;
        ...
        if cursor.seq > checkpoint_cursor.seq && proofs.is_empty() {
            return Err(JournalError::InvalidProof(
                "advancing a checkpoint requires confirmed send proofs".to_owned(),
            ));
        }
        for proof in proofs {
            if proof.seq <= checkpoint_cursor.seq || proof.seq > cursor.seq {
                return Err(JournalError::InvalidProof(
                    "proof sequence is outside the committed cursor range".to_owned(),
                ));
            }
            transaction.execute(
    ...
    pub fn consumer_cursor(&self, consumer_id: &str) -> JournalResult<Option<Cursor>> {
        ...
        let cursor = connection
            .query_row(
                "SELECT cursor FROM consumer_checkpoints WHERE consumer_id = ?1",
                [consumer_id],
...
pub(crate) fn append_in_transaction(
    ...
    Ok(Cursor::new(
        generation,
        sequence.try_into().map_err(|_| JournalError::Overflow)?,
    ))
}
...
fn validate_proofs(proofs: &[DeliveryProof]) -> JournalResult<()> {
    ...
    for proof in proofs {
        if !seen.insert(proof.seq) {
            return Err(JournalError::InvalidProof(
                "proof sequences must be unique".to_owned(),
            ));
        }
        if proof.dedupe_key.as_deref().is_some_and(str::is_empty) {
            return Err(JournalError::InvalidProof(
                "dedupe_key must not be empty when present".to_owned(),
            ));
        }
    }
    Ok(())
...
fn current_generation(connection: &rusqlite::Connection) -> JournalResult<u64> {
    let value = connection
        .query_row(
            "SELECT v FROM gateway_meta WHERE k = 'journal_generation'",
            [],
            |row| row.get::<_, String>(0),
        )
        .optional()?
        .ok_or_else(|| {
            JournalError::Store(StoreError::InvalidMetadata(
                "journal_generation is missing".to_owned(),
            ))
        })?;
    value.parse().map_err(|_| {
        JournalError::Store(StoreError::InvalidMetadata(
            "journal_generation is not a u64".to_owned(),
        ))
    })
}

fn current_generation_tx(transaction: &Transaction<'_>) -> JournalResult<u64> {
    let value = meta_get_tx(transaction, "journal_generation")?.ok_or_else(|| {
        JournalError::Store(StoreError::InvalidMetadata(
            "journal_generation is missing".to_owned(),
        ))
    })?;
...
fn retention_floor(connection: &rusqlite::Connection) -> JournalResult<u64> {
    let value = connection
        .query_row(
            "SELECT v FROM gateway_meta WHERE k = 'journal_floor_seq'",
            [],
            |row| row.get::<_, String>(0),
        )
        .optional()?
        .ok_or_else(|| {
            JournalError::Store(StoreError::InvalidMetadata(
                "journal_floor_seq is missing".to_owned(),
            ))
        })?;
    value.parse().map_err(|_| {
        JournalError::Store(StoreError::InvalidMetadata(
            "journal_floor_seq is not a u64".to_owned(),
        ))
    })
}
...
fn prune_in_transaction(transaction: &Transaction<'_>, now: i64) -> JournalResult<()> {
    let cutoff = now
        .checked_sub(EVENT_RETENTION_MS)
        .ok_or(JournalError::Overflow)?;
...
mod tests {
    ...
    use super::{
        Clock, ConsumerClaim, Cursor, DeliveryProof, EVENT_RETENTION_MS, EventJournal,
        JournalError, MAX_RETAINED_EVENTS, append_in_transaction,
    };
    ...
    fn journal() -> (EventJournal, Arc<FakeClock>) {
        ...
        (
            EventJournal::with_clock(Store::default(), clock.clone()),
            clock,
        )
    }
    ...
    fn retention_returns_a_typed_gap_and_keeps_a_resync_cursor() {
        ...
        let gap = read
            .gap
            .expect("cursor below the retention floor must receive a gap");
        ...
        assert_eq!(
            journal.read(Some(read.next_cursor), 10).unwrap().events[0].seq,
            retained.seq
        );
    }
    ...
    fn claim_is_exclusive_and_checkpoint_advances_only_when_committed() {
        ...
        let event = journal
            .append("surface_reply", "{\"text\":\"hi\"}")
            .unwrap();
        let claim = journal.claim_consumer("discord", Some(5_000)).unwrap();
        assert!(matches!(
            journal.claim_consumer("discord", Some(5_000)),
            Err(JournalError::ConsumerClaimHeld)
        ));
        assert_eq!(
            journal.consumer_cursor("discord").unwrap(),
            Some(Cursor::new(1, 0))
        );
    ...
    fn outbox_and_checkpoint_are_written_in_the_same_transaction() {
        ...
        let ConsumerClaim { claim_id, .. } =
            journal.claim_consumer("consumer", Some(5_000)).unwrap();
        journal
            .commit_consumer(
                "consumer",
                &claim_id,
    ...
    fn committed_checkpoint_and_outbox_are_durable_across_reopen() {
        ...
        let event = journal.append("surface_reply", "{}").unwrap();
        let claim = journal
            .claim_consumer("durable-consumer", Some(5_000))
            .unwrap();
        journal
            .commit_consumer(
                "durable-consumer",
                ...
                &[DeliveryProof {
                    seq: event.seq,
                    platform_msg_id: Some("platform-1".to_owned()),
                    dedupe_key: Some("d-1".to_owned()),
                }],
            )
            .unwrap();
        drop(journal);
        ...
        assert_eq!(
            reopened.consumer_cursor("durable-consumer").unwrap(),
            Some(event)
        );
