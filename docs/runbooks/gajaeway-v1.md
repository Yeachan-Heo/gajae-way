...
- **Session bootstrap remains pending:** inspect `session.list` or `ops.cycle`. A pending projection means the current epoch has not completed a terminal successful turn yet; pre-success failures deliberately retry it. Repeated attempts carry the same origin+epoch bootstrap ID. Do not mark it complete manually or copy source bodies into the database. Repair unreadable memory files or rejected links at their source. In public/group channels, associated channel/project/task/handoff documents need an exact `origin:`/`origin-key:`/`conversation-id:` match and `bootstrap-safe: public` (or `bootstrap-visibility: public`); daily entries are filtered to the exact current origin. The configured `memory/` root may itself be a symlink, but followed targets must remain beneath the resolved workspace or memory roots.
