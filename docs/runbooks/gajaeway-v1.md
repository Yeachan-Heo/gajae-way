...
The command is fail-closed by contract. `gates:` names reasons the cycle is not healthy, including `stale_session_identity`, `delivery_settlement_unknown`, `memory_closure_blocked`, `monitor_settlement_failed`, and `lane_capacity_exhausted`; any gate forces exit code 1, so automation can never read a degraded runtime as idle. `lanes: {active, max}` reports bound worker-lane capacity. An unavailable daemon is a connection error, not a healthy report. The projection is read-only; durable SQLite rows and the delivery ledger remain the authority.
...
- **`lane_capacity` on work.run / `lane_capacity_exhausted` in ops.cycle:** read `gajaeway work jobs` (one line per lane: the name `work retire` accepts, job state, `session_id`, `last_activity_at`), then retire the idlest lane with `gajaeway work retire <name>`. A refusal names why: an open attempt, a corrupt job record (repair `lane_jobs.record_json` at its source), an attempt the ledger ended without broker proof (`attempt_ended`/`terminal_uncertain`/`failed`; the lane is released once `gjc sdk session status` reports the op terminal or the session is dead), or `session.close failed and the session is not proven gone` (the broker is unreachable or the host is still alive; retry after the broker recovers, never delete sessions by hand). An open attempt is either a turn that is genuinely running (let it end) or one left open by a gateway that died mid-turn: nothing reconciles open attempts in the background, so for a job still `running` after a restart, issue `gajaeway work run <name> --cwd "<worktree_path printed by work jobs>" "<no-op>"` WITHOUT `--resume` (the cwd must match the lane's saved worktree or the request is refused before it touches the attempt); it closes the crash-left attempt as `terminal_uncertain`, holds the job (`awaiting_operator`), and runs nothing. Retirement then judges that attempt by broker status as above; `--resume` is the separate, explicit choice to continue the work instead. `lane_close_failed … action=retained` is the close refusal in the log; `action=session_gone` means the binding was cleared because liveness proved the host dead. Raise `work.maxLanes` in `config.json` only with a gateway restart; it is restart-required, not reloadable. `lane_retired … reason=idle|job_done|job_aborted` lines are the 60-second sweep working as designed.
