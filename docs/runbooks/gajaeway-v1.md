...
- **`work jobs` lists a lane `running` but its transcript stopped / `work steer` answers `steer acceptance uncertain` / `gjc sdk session status <id>` returns `session_unavailable`:** the lane's SDK host died with the attempt open. The observer settles it itself: once the router answers `session_unavailable` for the operation AND `session inspect` reports the id disowned or not live, continuously for 30s, the attempt ends `attempt_ended` with `errorCode=host_lost` (log: `work_host_lost opRef=… session=… source=observer|recovery`), the transcript and worktree are kept, and `work start <name> --resume` or `work retire <name>` then proceed. `broker_unavailable`, timeouts and other transport failures never settle an attempt; restore the broker first. `last=` in `work jobs` advances with streamed turn frames, so a lane whose `last=` stays put while `running` is stalled.
