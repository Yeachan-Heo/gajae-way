...
- `restartRequired` — fields you edited that are bound to a startup resource (`socketPath`, `dbPath`, `turnTimeoutMs`, `model`, `credentials`, `webhook`, `watcherRoots`, `scriptRoot`, `ownerTarget`, `monitorSessionTurnLimit`). They are reported and deliberately NOT applied; restart to pick them up.
