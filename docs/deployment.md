...
- `changed` — fields applied live. Only `mentionAllowlist`, `channels`, `debounceMs`, `dmPolicy`, and `handoffTargets` are re-read at runtime; a change to one of these takes effect on the next turn.
