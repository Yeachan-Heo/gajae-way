...
`list` prints a one-line-per-monitor table including `timezone` and `nextFire`; `--json` emits the monitor records, including `nextFireAt.local`, `nextFireAt.timezone`, and `nextFireAt.utc`. `--fields a,b,c` selects columns (an unknown name errors and lists the valid names) and `--limit N` / `--offset N` page the rows; both apply to `--json` as well. `sessions list` accepts the same flags. `inspect` returns the selected monitor, its next fire in the effective timezone and UTC, and recent event records. `test` submits an event and returns its `eventId`; omit `--type` to use the monitor’s first declared type. See [deployment](deployment.md) for `webhook`, `watcherRoots`, and `scriptRoot` configuration.
