...
assert(new TextDecoder().decode(commit.stdout).startsWith("145fa8f"));
assert.equal(report.sourceHash, sourceHash);
assert.equal(proof.sourceHash, sourceHash);
assert.equal(frozen.sourceHash, sourceHash);
...
assert.equal(report.adversarialCases.length, 63);
assert.deepEqual(report.adversarialCases.map((c: { id: string }) => c.id), Array.from({ length: 63 }, (_, i) => `RT-SLACK-${String(i + 1).padStart(2, "0")}`));
assert.deepEqual(report.adversarialCases.filter((c: { verdict: string }) => c.verdict === "failed").map((c: { id: string }) => c.id), ["RT-SLACK-54"]);
...
const baseline = Bun.spawnSync(["git", "show", "145fa8f:artifacts/slack-adapter-cli-replay.json"]);
...
const allowed = ["packages/adapter-slack/test/redteam.test.ts", "packages/gateway/test/slack-adapter-redteam.e2e.test.ts", "packages/sdk/test/client-held-events.test.ts"];
...
const receipt = { kind: "artifact-consistency-receipt", sourceHash, frozenCommit: "145fa8f", verdict: "passed", checks: ["JUnit test/failure counts match report", "all 63 IDs present exactly once", "only RT-SLACK-54 fails", "four real CLI probes pass", "build passes", "CLI replay byte-identical to frozen commit", "all artifact refs exist", "test snapshots byte-identical", "product source unchanged"], counts: report.counts };
