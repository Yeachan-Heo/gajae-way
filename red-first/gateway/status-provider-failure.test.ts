...
test("red 6: repeated provider failures remain visible when an active probe disagrees", async () => {
	const port = new ScriptedSessionPort({ onSend: (input, scripted) => scripted.fail(input.opRef, "Prompt submission failed.") });
	...
	const real = new BrokerSessionPort({ database: h.database, cli: run, instanceId: "red-provider", tailRunner: new TailRunner({ run }) });
	port.status = real.status.bind(real);
	let probeStatus = 403;
	const probe = Bun.serve({ port: 0, fetch: () => new Response("fixture provider", { status: probeStatus }) });
	const server = await startUnixServer({ database: h.database, sessionPort: port, config: {
		schemaVersion: 1, home: h.home, configPath: `${h.home}/config.json`, socketPath: `${h.home}/gateway.sock`, dbPath: `${h.home}/gateway.db`, logVerbosity: "info",
		dmPolicy: "open",
	} });
	const frames: Array<{ id?: string; result?: any; payload?: { text?: string } }> = [];
	let buffered = "";
	const socket = await Bun.connect({ unix: `${h.home}/gateway.sock`, socket: { data(_socket, data) {
		buffered += new TextDecoder().decode(data);
		let newline: number;
		while ((newline = buffered.indexOf("\n")) >= 0) { frames.push(JSON.parse(buffered.slice(0, newline))); buffered = buffered.slice(newline + 1); }
	} } });
	...
	try {
		...
		for (let i = 0; i < 2; i++) {
			send({ v: "0.1", type: "request", id: `turn-${i}`, verb: "chat.send", params: { origin: { platform: "discord", kind: "dm", conversationId: "red-provider", peerId: "fixture" }, text: `fail-${i}`, engagement: { mentioned: false, group: false, authorId: "fixture" } } });
			await eventually(() => frames.filter(f => f.payload?.text?.startsWith("[turn failed]")).length >= i + 1, "failure notice missing");
		}
	...
	} finally { socket.end(); await server.stop(); probe.stop(true); await h.close(); }
});
