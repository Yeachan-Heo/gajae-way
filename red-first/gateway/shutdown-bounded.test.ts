...
test("red 2: SIGTERM exits an accepted-turn daemon within 15 seconds", async () => {
	...
	const child = Bun.spawn({
		cmd: [process.execPath, "packages/gateway/src/main.ts", "daemon"],
		cwd: resolve("."),
		env: {
			...process.env,
			HOME: home,
			...
			GAJAEWAY_FAKE_GJC_MODES: "status:hang-30s",
			RED_FIRST_CHILD_PIDS: join(home, "children"),
			RED_FIRST_STATUS_MARKER: join(home, "status-entered"),
		},
		stdin: "ignore",
		stdout: "pipe",
		stderr: "pipe",
	...
	try {
		for (let i = 0; i < 600; i++) {
			try {
				socket = await Bun.connect({ unix: join(home, "gateway.sock"), socket: { data() {} } });
				break;
			} catch {
				if (child.exitCode !== null) throw new Error(`daemon exited before socket: ${output}`);
				await Bun.sleep(25);
			}
		}
		...
		socket.write(
			`${JSON.stringify({ v: "0.1", type: "request", id: "accepted", verb: "chat.send", params: { origin: ORIGIN, text: "remain accepted" } })}\n`,
		);
		await eventually(
			() => database.inboundNonterminalTurns(KEY).some((row) => row.state === "accepted"),
			`accepted turn not seeded: ${output}`,
			15_000,
		);
