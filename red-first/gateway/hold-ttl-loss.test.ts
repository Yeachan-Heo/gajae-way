...
test("red 3: an accepted unknown operation becomes lost after 31 minutes without replay", async () => {
	...
	const real = new BrokerSessionPort({ database: h.database, cli: run, instanceId: "red-hold", tailRunner: new TailRunner({ run }) });
	...
	try {
		...
		h.database.inboundEnqueue({ messageId: "lost", originKey: KEY, originRefJson: JSON.stringify(ORIGIN), body: "lost" });
		h.database.inboundBindTurn({ messageId: "lost", originKey: KEY, epoch: 0, opRef: "gw-red-lost", sessionId: binding.sessionId });
