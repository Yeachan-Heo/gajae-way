...
import { DISCORD_CONSUMER_ID } from "../../src/adapter/discord/config";
import { startDiscordAdapter } from "../../src/adapter/discord/main";
import type { DiscordPlatform } from "../../src/adapter/discord/platform";
import { discordAdapterPlatform } from "../../src/adapter/discord/port";
import { AdapterEgress, adapterDedupeKey, wireNonce } from "../../src/adapter/runtime/egress";
...
import { createExternalGateway, type ExternalGateway, eventually } from "../helpers/external-gateway";
...
/**
 * Test-local bindings onto the shared adapter runtime.
 *
 * The delivery logic these previously exercised now lives in
 * `src/adapter/runtime/`, so these keep the existing assertions pointed at the
 * shipped code path instead of a deleted per-adapter implementation.
 */
function routeHandler(options: {
	readonly rpc: JsonRpcClient;
	readonly platform: DiscordPlatform;
	readonly route: { readonly channelId: string; readonly surfaceId: string };
	readonly acknowledgement?: { readonly budgetMs?: number; readonly now?: () => number };
	onAccepted?(message: { readonly id: string }, journalHeadCursor: unknown): void;
	onAcknowledged?(acknowledgement: { readonly messageId: string }): void;
	onAcknowledgementDiagnostic?(message: { readonly id: string }, reason: string): void;
}) {
	const port = discordAdapterPlatform(options.platform, options.route.channelId);
	const ingress = new AdapterIngress({
		rpc: options.rpc,
		platform: port,
		surfaceId: options.route.surfaceId,
		chatId: options.route.channelId,
		...(options.acknowledgement?.budgetMs === undefined ? {} : { ackBudgetMs: options.acknowledgement.budgetMs }),
		...(options.acknowledgement?.now === undefined ? {} : { now: options.acknowledgement.now }),
		...(options.onAccepted === undefined
			? {}
			: { onAccepted: (message, cursor) => options.onAccepted?.({ id: message.platformMsgId }, cursor) }),
		...(options.onAcknowledged === undefined
			? {}
			: { onAcknowledged: (ack) => options.onAcknowledged?.({ messageId: ack.platformMsgId }) }),
		...(options.onAcknowledgementDiagnostic === undefined
			? {}
			: {
					onAcknowledgementDiagnostic: (message, reason) =>
						options.onAcknowledgementDiagnostic?.({ id: message.platformMsgId }, reason),
				}),
	});
	return {
...
function discordDedupeKey(surfaceId: string, seq: string): string {
	return adapterDedupeKey(DISCORD_CONSUMER_ID, surfaceId, seq);
}

function discordWireNonce(dedupeKey: string): string {
	return wireNonce(dedupeKey);
}
...
function outbox(
	...
	hooks: ConstructorParameters<typeof AdapterEgress>[0]["hooks"] = undefined,
): AdapterEgress {
	return new AdapterEgress({
		rpc: gatewayUnderTest.client,
		platform: discordAdapterPlatform(fixture, "123456789012345678"),
...
function typingAdapterRpc(
	accepted: boolean,
	acceptedResponse: Record<string, unknown> = accepted
		? { accepted: true, journal_head_cursor: "1:0" }
		: { accepted: false },
): JsonRpcClient {
	let claimCount = 0;
	return {
			...
			switch (method) {
				...
				case "consumer.claim":
					...
					return typingRpcResult({
						claim_id: `typing-claim-${claimCount}`,
						cursor: "1:0",
...
externalTest(
	"Discord inbound deduplicates a message id before external broker admission and sends finalized output",
	async () => {
		...
		try {
			...
			await eventually(
				() =>
					gatewayUnderTest.core.journalRead("1:0", 20).events.some((event) => event.kind === "assistant_message")
						? true
						: undefined,
...
externalTest("Discord typing keepalive refreshes a slow accepted turn and stops after reply delivery", async () => {
	...
	try {
		...
		const inbound = {
			...
			text: "keep typing until the reply arrives",
		};
		await fixture.emitMessage(inbound);
		const opRef = await eventually(() => {
			const command = gatewayUnderTest.fixture
				.commands()
...
externalTest("Discord typing keepalive ignores an older delayed delivery but stops at a later reply", async () => {
	...
	try {
		...
		const older = gatewayUnderTest.core.journalAppend(
			"assistant_message",
			JSON.stringify({ finalized: true, text: olderText }),
		);
		await eventually(
			() => (fixture.pendingSendCount === 1 ? true : undefined),
			"old Discord outbox send was not paused",
		);

		gatewayUnderTest.fixture.holdNextTurn();
		const inbound = {
			id: "typing-causal-new",
			channelId: TYPING_CHANNEL_ID,
			text: "new accepted command survives old delivery",
		};
		await fixture.emitMessage(inbound);
		const opRef = await eventually(() => {
			const command = gatewayUnderTest.fixture
		...
		await eventually(
			() => (fixture.sends.some((send) => send.text === replyText) ? true : undefined),
			"new assistant reply was not delivered",
		);
		await eventually(
			() => (clock.scheduledTimerCount === 0 ? true : undefined),
			"newer delivery did not stop the typing keepalive",
		);
...
externalTest(
	"Discord typing keepalive stops for a reply journaled before the accepted response reaches the adapter",
	async () => {
		const gatewayUnderTest = await gateway();
		const clock = new DiscordFixtureClock(1_000);
		const fixture = new DiscordFixture({ now: clock.now });
		...
		const adapterRpc: JsonRpcClient = {
			async request(method, params, options) {
				...
				const result = response.result as
					| { accepted?: unknown; op_ref?: unknown; journal_head_cursor?: unknown }
					| undefined;
		...
		try {
			...
			await fixture.emitMessage({
				id: "typing-fast-reply",
				channelId: TYPING_CHANNEL_ID,
				text: "respond before adapter observes acceptance",
			});
			expect(serverBoundary).toMatch(/^\d+:\d+$/);
			await eventually(
				() => (fixture.pendingSendCount === 1 ? true : undefined),
				"fast journaled reply was not held before Discord delivery",
			);
			...
			await eventually(
				() => (fixture.sends.some((send) => send.text === replyText) ? true : undefined),
				"fast reply was not delivered to Discord",
			);
			await eventually(
				() => (clock.scheduledTimerCount === 0 ? true : undefined),
				"fast reply did not stop the typing keepalive",
			);
			await advanceTypingClock(clock, 30_000);
			expect(fixture.acknowledgements).toEqual([{ channelId: TYPING_CHANNEL_ID, at: 1_000 }]);
		} finally {
...
externalTest(
	"Discord typing keepalive shares one channel interval and extends its cap for a later admission",
	async () => {
		const clock = new DiscordFixtureClock(1_000);
		const fixture = new DiscordFixture({ now: clock.now });
		...
		try {
			...
			await fixture.emitMessage({
				id: "typing-second",
				channelId: TYPING_CHANNEL_ID,
				text: "second accepted admission",
			});
...
externalTest(
	"Discord typing keepalive diagnoses missing and malformed admission boundaries while retaining typing",
	async () => {
		for (const scenario of [
			{ name: "missing", response: { accepted: true }, detail: "omitted journal_head_cursor" },
			{
				name: "malformed",
				response: { accepted: true, journal_head_cursor: "not-a-cursor" },
				detail: "invalid journal_head_cursor",
			},
		] as const) {
			const clock = new DiscordFixtureClock(1_000);
			const fixture = new DiscordFixture({ now: clock.now });
			const errors: Error[] = [];
			const adapter = await startTypingFixtureAdapter(
				fixture,
				clock,
				typingAdapterRpc(true, scenario.response),
...
externalTest(
	"Discord typing keepalive diagnoses a stale-generation admission boundary while retaining typing",
	async () => {
		const gatewayUnderTest = await gateway();
		const clock = new DiscordFixtureClock(1_000);
		const fixture = new DiscordFixture({ now: clock.now });
		...
		try {
			gatewayUnderTest.fixture.holdNextTurn();
			const inbound = {
				id: "typing-stale-generation",
				channelId: TYPING_CHANNEL_ID,
				text: "stale generation must retain typing",
			};
			await fixture.emitMessage(inbound);
			const opRef = await eventually(() => {
				const command = gatewayUnderTest.fixture
			...
			await eventually(
				() => (fixture.sends.some((send) => send.text === "stale generation reply") ? true : undefined),
				"stale-boundary reply was not delivered",
			);
			await eventually(
...
externalTest("Discord typing keepalive retries after a non-fatal typing failure", async () => {
	...
	const fixture = new DiscordFixture({ now: clock.now });
	const errors: Error[] = [];
	const adapter = await startTypingFixtureAdapter(fixture, clock, typingAdapterRpc(true), (error) =>
		errors.push(error),
	);
	try {
		...
		expect(errors).toEqual([
			expect.objectContaining({
				message: `Discord typing keepalive failed for channel ${TYPING_CHANNEL_ID}: fixture typing acknowledgement failed`,
			}),
		]);
...
externalTest("Discord typing keepalive allows only one deferred refresh per channel", async () => {
	...
	try {
		await fixture.emitMessage({
			id: "typing-deferred-refresh",
			channelId: TYPING_CHANNEL_ID,
			text: "hold one refresh request",
		});
		await clock.flushAsync();
...
externalTest("Discord typing keepalive does not reschedule or report after stop during a refresh", async () => {
	...
	try {
		await fixture.emitMessage({
			id: "typing-stop-pending",
			channelId: TYPING_CHANNEL_ID,
			text: "stop pending refresh",
		});
		await clock.flushAsync();
		fixture.deferNextTyping();
		await advanceTypingClock(clock, 8_000);
...
externalTest("Discord typing follows accepted admission, not a stale healthy-to-fenced connection", async () => {
	...
	try {
		...
		if (typeof intent.op_ref !== "string")
			throw new Error("fenced Discord setup did not retain its operation reference");
		await eventually(
		...
		const blocked = {
			id: "discord-fenced-message",
			channelId: "123456789012345678",
			text: "must remain unacknowledged",
		};
		await expect(route.handle({ ...blocked, acceptedAt: Date.now() })).rejects.toMatchObject({
		...
		const fresh = {
			id: "discord-after-promotion",
			channelId: "123456789012345678",
			text: "fresh ingress after promotion",
			acceptedAt: Date.now(),
		};
...
externalTest(
	"Discord startup reports rate-limited verifying and transport readiness diagnostics without opening ingress",
	async () => {
		const config = {
		...
		expect(verifyingDiagnostics).toEqual([
		...
		await eventually(
			() => (connectionAttempts >= 3 ? true : undefined),
			"adapter did not retry the unavailable gateway transport",
		);
		transportAbort.abort();
		await expect(transportStart).rejects.toMatchObject({ name: "AbortError" });
		expect(transportDiagnostics).toEqual([
			"gateway transport/protocol error while waiting: could not connect to the local gateway: fixture UDS unavailable",
		]);
	},
);
...
externalTest("Discord outbox settles server-side delivery before a restart can repost", async () => {
	...
	await fixture.connect();
	try {
		const appended = gatewayUnderTest.core.journalAppend(
			"assistant_message",
			JSON.stringify({ finalized: true, text: "settle this" }),
		);
		expect(await outbox(gatewayUnderTest, fixture).runOnce()).toBe("sent");
		...
		expect(gatewayUnderTest.core.consumerOutbox("gajaeway-discord")).toEqual([
			expect.objectContaining({
				seq: appended.seq,
				state: "sent",
				dedupeKey: `gajaeway-discord:discord:owner-dm:${appended.seq}`,
			}),
		]);
...
externalTest(
	"Discord crash before send leaves the durable checkpoint for restart delivery",
	async () => {
		const gatewayUnderTest = await gateway();
		const fixture = new DiscordFixture();
		await fixture.connect();
		try {
			const appended = gatewayUnderTest.core.journalAppend(
				"assistant_message",
				JSON.stringify({ finalized: true, text: "retry before send" }),
			);
...
externalTest(
	"Discord crash after send before settlement retries with a nonce and does not double-post",
	async () => {
		const gatewayUnderTest = await gateway();
		const fixture = new DiscordFixture();
		await fixture.connect();
		try {
			const appended = gatewayUnderTest.core.journalAppend(
				"assistant_message",
				JSON.stringify({ finalized: true, text: "retry after send" }),
			);
			const abort = new AbortController();
...
externalTest(
	"Discord non-owner engagement is admitted as follow_up whether the external turn is idle or busy",
	async () => {
		const gatewayUnderTest = await gateway();
		const fixture = new DiscordFixture();
		await fixture.connect();
		try {
			const guestRoute = routeHandler({
				rpc: gatewayUnderTest.client,
				platform: fixture,
				route: { channelId: "222222222222222222", surfaceId: "discord:guest-channel" },
			});
			await guestRoute.handle({
				id: "guest-idle",
			...
			gatewayUnderTest.fixture.holdNextTurn();
			const ownerRoute = routeHandler({
				rpc: gatewayUnderTest.client,
				platform: fixture,
				route: { channelId: "123456789012345678", surfaceId: "discord:owner-dm" },
			});
			await ownerRoute.handle({
				id: "owner-held",
				channelId: "123456789012345678",
				text: "held owner message",
				acceptedAt: Date.now(),
			});
			await eventually(
				() => (gatewayUnderTest.host.turnState === "busy" ? true : undefined),
				"owner turn was not admitted as busy",
			);
			await guestRoute.handle({
				id: "guest-busy",
				channelId: "222222222222222222",
			...
			expect(
				gatewayUnderTest.fixture.commands().map((command) => ({ operation: command.operation, text: command.text })),
			).toEqual([
			...
			if (typeof heldPrompt?.opRef !== "string")
				throw new Error("held Discord owner prompt was not recorded with an operation reference");
					...
					gatewayUnderTest.core
						...
						.events.some(
							(event) =>
								...
								JSON.parse(event.payloadJson).text === "settled after Discord follow-up assertion",
						)
						? true
						: undefined,
...
/**
 * The operator-reported defect: typing never appeared.
 *
 * A typing failure on the ACK path used to be fatal - it discarded the keepalive
 * admission and threw out of message handling - so the gateway durably accepted
 * the turn and replied while the operator never saw an indicator at all. The
 * tick path was already tolerant; the ack path was not, and that asymmetry had
 * no test.
 */
externalTest("a typing failure on the ack path degrades to a diagnostic and still starts the keepalive", async () => {
	const clock = new DiscordFixtureClock(1_000);
	const fixture = new DiscordFixture({ now: clock.now });
	const errors: Error[] = [];
	const adapter = await startTypingFixtureAdapter(fixture, clock, typingAdapterRpc(true), (error) =>
	...
	try {
		// Fail the very first typing POST, i.e. the one issued right after the
		// durable acceptance.
		fixture.failNextTyping();
		await fixture.emitMessage({
			id: "typing-ack-degrade",
			channelId: TYPING_CHANNEL_ID,
			text: "ack path typing fails",
		});

		// Reported, not thrown.
		await eventually(
			() => (errors.some((error) => error.message.includes("Discord typing unavailable")) ? true : undefined),
			"an ack-path typing failure did not surface a diagnostic",
		);

		// The keepalive still started despite that failure, so the operator sees
		// activity for the turn that is genuinely in flight.
		await advanceTypingClock(clock, 8_000);
		expect(fixture.acknowledgements.length).toBeGreaterThan(0);
	} finally {
		await adapter.stop();
	}
});
