...
import { dispatchMcp } from "../../src/mcp/serve";
import {
	connectEventually,
	createExternalGateway,
	type ExternalGateway,
	eventually,
} from "../helpers/external-gateway";
...
async function waitForBusyStatus(
	gateway: ExternalGateway,
): Promise<Awaited<ReturnType<ExternalGateway["client"]["request"]>>> {
...
async function settleHeld(gateway: ExternalGateway, opRef: string, text: string): Promise<void> {
	...
	const terminalEventsBefore = gateway.core
		.journalRead("1:0", 100)
		.events.filter((event) => event.kind === "turn_end").length;
	gateway.fixture.complete(opRef, { text });
	await eventually(
		() =>
			gateway.core.journalRead("1:0", 100).events.filter((event) => event.kind === "turn_end").length >
...
externalTest("main.submit admits a held external turn before the bridge timeout", async () => {
	...
	expect(gateway.fixture.commands()).toEqual([
		expect.objectContaining({ operation: "turn.prompt", text: "held owner prompt" }),
	]);
...
externalTest(
	"pending busy adoption records a visible delivery gap when its first complete tail includes a post-commit reply",
	async () => {
		...
		try {
			...
			expect(gateway.state.read()).toMatchObject({
				...
				transcriptDeliveryProgress: {
					lastEntryId: `${fixture.sessionId}:transcript:3`,
					fingerprint: { entryCount: 4 },
				},
			});
			...
			expect(
				boundaryEvents.filter(
					(event) => event.kind === "turn_start" || event.kind === "turn_end" || event.kind === "assistant_message",
				),
			).toEqual([]);
			...
			await eventually(
				() =>
					gateway.core
						.journalRead("1:0", 100)
						.events.some(
							(event) => event.kind === "assistant_message" && event.payloadJson.includes("proof-bound roundtrip"),
						)
						? true
						: undefined,
...
externalTest(
	"successive attempt generations project once and settle as ordered tail coordinates",
	async () => {
		...
		for (const round of rounds) {
			...
			await eventually(
				() =>
					gateway.core
						.journalRead("1:0", 100)
						.events.some((event) => event.kind === "assistant_message" && event.payloadJson.includes(round.reply))
						? true
						: undefined,
				`generation ${round.generation} transcript reply did not project`,
			);
			await eventually(
				() => (gateway.host.turnState === "idle" ? true : undefined),
				`generation ${round.generation} did not settle`,
			);
			expect(gateway.state.read().tailCheckpoint).toMatchObject({ generation: round.generation, seq: 5 });
		}

		const lifecycle = gateway.core
			.journalRead("1:0", 100)
		...
		for (const round of rounds) {
			expect(
				gateway.core
					.journalRead("1:0", 100)
					.events.filter((event) => event.kind === "assistant_message" && event.payloadJson.includes(round.reply)),
			).toHaveLength(1);
		}
		expect(gateway.state.read()).toMatchObject({
		...
		expect(
			gateway.core.journalRead("1:0", 100).events.filter((event) => event.kind === "tail_ring_rotation"),
		).toHaveLength(0);
...
externalTest(
	"busy ring rotation across an attempt-generation boundary resyncs without losing transcript delivery",
	async () => {
		const gateway = await hosted({ tailTimeoutMs: 50 });
		...
		await eventually(
			() => (gateway.host.turnState === "idle" ? true : undefined),
			"first attempt did not settle before the generation-boundary rotation",
		);
		...
		await eventually(
			() =>
				gateway.core
					...
					.events.some(
						(event) =>
							event.kind === "assistant_message" && event.payloadJson.includes("final reply after busy ring rotation"),
					)
					? true
					: undefined,
		...
		await eventually(
			() =>
				gateway.core
					.journalRead("1:0", 100)
					.events.some(
						(event) => event.kind === "assistant_message" && event.payloadJson.includes("post-rotation roundtrip"),
					)
					? true
					: undefined,
...
externalTest(
	"restart replays the exact verification tail after a down-time ring rotation without rebaselining transcript delivery",
	async () => {
		const gateway = await hosted({ tailTimeoutMs: 50 });
		gateway.fixture.setNoEnvelopeWhileBusy();
		gateway.fixture.holdNextTurn();
		...
		try {
			...
			expect(resumed.verificationTail).toMatchObject({
				retentionGap: true,
			...
			restartedHost = createMainSessionHost({
				...
				journal: {
					...
					journalAppendTranscriptProjection: (
						...
						gateway.state.appendTranscriptProjection(
							expectedTail,
							checkpoint,
							expectedDelivery,
							nextDelivery,
					...
					gateway.core
						...
						.events.find(
							(event) =>
								...
								event.payloadJson.includes("reply recovered from verification-tail delivery"),
						),
...
externalTest(
	"restart reconciliation preserves receipt-bound surface attribution for a downtime-finalized reply",
	async () => {
		const gateway = await hosted({
			tailTimeoutMs: 50,
		...
		expect(gateway.core.mainAdmissionAttributions()).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ surfaceId: "owner", attemptIdsJson: expect.stringContaining(opRef) }),
			]),
		);
		await gateway.host.dispose();
		gateway.fixture.rotateRingDuringNextCompletion();
			...
			const reply = await eventually(
				() =>
					gateway.core
						.journalRead("1:0", 100)
						.events.find(
							(event) => event.kind === "assistant_message" && event.payloadJson.includes("reconciled downtime reply"),
						),
...
externalTest(
	"a transcript prefix break remains a fail-closed authority violation",
	async () => {
		const gateway = await hosted();
		gateway.fixture.holdNextTurn();
		const accepted = await gateway.client.request("main.submit", {
		...
		expect(gateway.state.read()).toMatchObject({
			...
			failedClosedReason: "growth_intent_mismatch",
			transcriptDeliveryGapCount: 1,
		});
		...
		expect((gaps.result as { events: Array<{ kind: string; payload: unknown }> }).events).toEqual([
			expect.objectContaining({
				...
				payload: expect.objectContaining({ reason: "transcript_delivery_unprovable" }),
			}),
		]);
	},
	30_000,
...
externalTest(
	"main.submit returns delivered_as before a held external assistant is journaled",
	async () => {
		const gateway = await hosted();
		gateway.fixture.holdNextTurn();
		...
		const assistant = await eventually(
			() =>
				gateway.core
					...
					.events.find(
						(event) =>
							event.kind === "assistant_message" && JSON.parse(event.payloadJson).text === "journaled after admission",
					),
...
externalTest("main.submit replays a pending admission identically without double-sending", async () => {
	...
	expect(
		gateway.fixture
			.commands()
			.filter((command) => command.operation === "turn.prompt" && command.text === "pending once"),
	).toHaveLength(1);
	await settleHeld(gateway, (first.result as { op_ref: string }).op_ref, "settled after idempotent replay assertion");
});

externalTest(
	"main.submit preserves an ambiguous post-acceptance claim for reconciliation without a second send",
	async () => {
...
externalTest(
	"a post-acceptance bridge error retains broker receipt linkage until nonterminal tail evidence finalizes its claim",
	async () => {
		const gateway = await hosted({
			afterBrokerAcceptedBeforeFinalize: () => {
		...
		gateway.fixture.holdNextTurn();
		const request = {
			text: "receipt-linked terminal recovery",
			surface_id: "owner",
			idempotency_key: "receipt-linked-terminal",
		};
		const interrupted = await gateway.client.request("main.submit", request);
		expect(rpcError(interrupted)).toMatchObject({ code: -32603, message: "bridge_exception" });
		const [pending] = gateway.core.mainAdmissionOperationsPending();
		const intent = JSON.parse(pending?.intentJson ?? "{}") as { op_ref?: unknown };
		if (typeof intent.op_ref !== "string")
			throw new Error("post-acceptance failure did not retain a durable operation reference");
		const opRef = intent.op_ref;
		...
		const terminal = gateway.core
			.journalRead("1:0", 100)
			.events.find(
				(event) => event.kind === "turn_end" && JSON.parse(event.payloadJson).attempt_id === `broker-turn-${opRef}`,
			);
...
externalTest(
	"an accepted operation with a lost receipt preserves growth authorization across restart without resend",
	async () => {
		const gateway = await hosted({ tailTimeoutMs: 50 });
		gateway.fixture.setNoEnvelopeWhileBusy();
		gateway.fixture.holdNextTurn();
		...
		const request = {
			text: "accepted before receipt loss",
			surface_id: "owner",
			idempotency_key: "lost-receipt-growth",
		};
		const interrupted = await gateway.client.request("main.submit", request);
		expect(rpcError(interrupted)).toMatchObject({
			code: -32603,
			message: "bridge_exception",
			data: { reason: "turn_admission_failed" },
		});
		expect(
			gateway.fixture
				.commands()
		...
		try {
				...
				() =>
					gateway.core
						...
						.events.some(
							(event) =>
								event.kind === "assistant_message" &&
								event.payloadJson.includes("recovered after accepted receipt loss"),
						)
						? true
						: undefined,
			...
			await eventually(
				() => (gateway.state.read().growthIntent === undefined ? true : undefined),
				"lost-receipt recovery did not settle durable growth authorization",
			);
...
externalTest(
	"terminal tail evidence finalizes fast same-process lost-receipt claims before their send failures and never accumulates rows",
	async () => {
		...
		const settleLostReceipt = async (ordinal: number): Promise<void> => {
			...
			await eventually(
				() =>
					gateway.host.mutationReadinessReason === undefined && gateway.state.read().growthIntent === undefined
						? true
						: undefined,
				"terminal evidence did not settle the fast receipt-loss admission",
			);
			const commands = gateway.fixture
				.commands()
				.filter((command) => command.operation === "turn.prompt" && command.text === request.text);
			expect(commands).toHaveLength(1);
			if (typeof commands[0]?.opRef !== "string")
				throw new Error("fast same-process lost-receipt admission did not retain its operation reference");
			const replay = await gateway.client.request("main.submit", request);
			expect(replay.result).toMatchObject({
				accepted: true,
...
externalTest(
	"a definitive broker rejection abandons only its claim while ambiguous accepted claims still reconcile without resend",
	async () => {
		const gateway = await hosted();
		const rejectedRequest = {
			text: "retry after definite rejection",
			surface_id: "owner",
		...
		expect(rpcError(rejected)).toMatchObject({
			code: -32603,
			message: "bridge_exception",
			data: { reason: "turn_admission_failed" },
		});
		await eventually(
			() => (gateway.state.read().growthIntent === undefined ? true : undefined),
			"definitive rejection did not clear its growth intent",
		);
...
externalTest("main.submit rejects conflicting reuse of a pending idempotency key with 1500", async () => {
	...
	expect(rpcError(conflict)).toEqual(expect.objectContaining({ code: 1500, message: "idempotency_conflict" }));
	expect(
		gateway.fixture.commands().filter((command) => command.opRef === (accepted.result as { op_ref: string }).op_ref),
	).toHaveLength(1);
...
externalTest("main.submit rejects empty text before broker admission", async () => {
	...
	expect(rpcError(response)).toEqual(
		expect.objectContaining({ code: -32602, message: "text must be a non-empty string." }),
	);
	expect(gateway.fixture.commands()).toEqual([]);
});
...
externalTest("non-owner admission is follow_up while the external main session is idle or busy", async () => {
	...
	expect(
		gateway.fixture
			.commands()
			.filter((command) => command.operation === "turn.follow_up")
			.map((command) => command.text),
	).toEqual(["idle guest message", "busy guest message"]);
...
externalTest("post-admission external failure degrades health and closes the terminal journal attempt", async () => {
	...
	await eventually(
		() => (gateway.host.degraded ? true : undefined),
		"host did not degrade after terminal broker failure",
	);
	await eventually(
	...
	const terminalEvents = gateway.core
		.journalRead("1:0", 100)
		.events.filter(
			(event) => event.kind === "turn_start" || event.kind === "assistant_message" || event.kind === "turn_end",
		);
...
externalTest(
	"main.events.read surface-attributes an owner admission lifecycle and transcript reply",
	async () => {
		const gateway = await hosted();
		gateway.fixture.holdNextTurn();
		const response = await gateway.client.request("main.submit", {
		...
		await eventually(
			() =>
				gateway.core.journalRead("1:0", 100).events.filter((event) => event.kind === "turn_end").length === 1
					? true
					: undefined,
		...
		expect(rows[0]?.payload).toEqual({
			attempt_id: `${gateway.fixture.sessionId}:${opRef}`,
			generation: 1,
			lineage: "main",
			surface_id: "owner",
		});
...
externalTest("two configured admission surfaces retain distinct attribution across separate turns", async () => {
	...
	expect(
		rows.find((row) => row.kind === "assistant_message" && row.payload.text === "owner-origin reply")?.payload,
	).toMatchObject({ surface_id: "owner" });
	expect(
		rows.find((row) => row.kind === "assistant_message" && row.payload.text === "guest-origin reply")?.payload,
	).toMatchObject({ surface_id: "guest" });
});
...
externalTest("unmatched autonomous main lifecycle events remain unattributed", async () => {
	...
	gateway.fixture.appendTailEvent("turn_end", {
		...
		message: {
			role: "assistant",
			content: [{ type: "text", text: "autonomous completion" }],
			responseId,
			turnId: attemptId,
		},
		scope: { attemptId, generation: 1, lineage: "main" },
	...
	const rows = await eventually(() => {
		...
		return events.some((event) => event.kind === "turn_end" && event.payloadJson.includes(attemptId))
			? events
			: undefined;
	...
	const terminal = await eventually(
		() =>
			gateway.core
				.journalRead("1:0", 100)
				.events.find(
					(event) => event.kind === "turn_end" && JSON.parse(event.payloadJson).attempt_id === suffixAttemptId,
				),
...
externalTest("main.gate.answer reports unsupported and durably replays that honest broker limitation", async () => {
	...
	try {
		...
		expect(
			...
			expect.objectContaining({
				replayed: true,
				responseJson: canonicalJson({ accepted: false, gate_state: "unsupported" }),
			}),
		);
	} finally {
		replayClient.close();
...
externalTest(
	"process tail exhaustion keeps the RPC socket serving while every health surface reports degraded",
	async () => {
		const fixture = new FakeBrokerFixture();
...
/**
 * The named P0-1 falsifier for D1: a `system_event` run must enqueue behind the
 * operator turn with NO surface entry present in the profile at all, proving the
 * scheduler never borrows admission authority from configured routing.
 *
 * This asserts the operator-visible observable (`way.status`), not just the
 * internal delivery classification.
 */
externalTest("a scheduled system_event enqueues follow_up with no surface entry in the profile", async () => {
	const gateway = await hosted();

	// The profile declares an owner surface plus the `guest` known surface; the
	// scheduler job below declares neither.
	const created = await gateway.client.request("schedule.create", {
		idempotency_key: "sched-status-1",
		name: "status nudge",
		kind: "every",
		spec: String(60_000),
		timezone: "UTC",
		payload_kind: "system_event",
		payload: { text: "scheduled system event enqueues" },
	});
	const createdResult = created.result as { job_id: string };
	expect(typeof createdResult.job_id).toBe("string");

	const job = (await gateway.client.request("schedule.get", { job_id: createdResult.job_id })).result as {
		surfaceId?: string;
		payloadKind: string;
	};
	// The durable row carries no surface at all.
	expect(job.payloadKind).toBe("system_event");
	expect(job.surfaceId ?? null).toBeNull();

	gateway.fixture.holdNextTurn();
	await gateway.client.request("main.submit", {
		text: "operator turn holds the session",
		surface_id: "discord:owner-dm",
		idempotency_key: "sched-status-operator",
	});

	await gateway.client.request("schedule.run_now", {
		idempotency_key: "sched-status-run",
		job_id: createdResult.job_id,
	});

	await eventually(async () => {
		const status = (await gateway.client.request("way.status", {})).result as {
			follow_up_queue_depth?: number;
		};
		return (status.follow_up_queue_depth ?? 0) > 0 ? true : undefined;
	}, "a scheduled system_event did not raise follow_up_queue_depth");

	const runs = (await gateway.client.request("schedule.runs", { job_id: createdResult.job_id })).result as {
...
/**
 * The MCP read tools must never move a durable consumer checkpoint.
 *
 * `journal_read` and `transcript_read` are projections of `main.events.read`; if
 * either passed a `consumer_id`, it would advance a real adapter's checkpoint
 * and that adapter would silently never receive the skipped events. This drives
 * the tools against a real daemon and asserts `way.status.consumers` is
 * byte-identical afterwards.
 */
externalTest("MCP read tools leave way.status consumers unchanged", async () => {
	const gateway = await hosted();

	// Establish a real consumer checkpoint first, so there is something to move.
	const claim = (
		await gateway.client.request("consumer.claim", { consumer_id: "gajaeway-discord", claim_ttl_ms: 5_000 })
	).result as { claim_id: string; cursor: string };
	await gateway.client.request("consumer.commit", {
		consumer_id: "gajaeway-discord",
	...
	const before = JSON.stringify(
		((await gateway.client.request("way.status", {})).result as { consumers?: unknown }).consumers,
	);

	const rpc = {
		close: () => {},
		request: async (method: string, params?: unknown) => await gateway.client.request(method, params),
	};
	for (const name of ["journal_read", "transcript_read"]) {
		const response = (await dispatchMcp(
			{ jsonrpc: "2.0", id: 1, method: "tools/call", params: { name, arguments: { cursor: "1:0" } } },
			// biome-ignore lint/suspicious/noExplicitAny: the harness client satisfies the read surface.
			{ rpc: rpc as any, writeEnabled: false, serverVersion: "test" },
...
/**
 * The MCP tools must inherit fail-closed from their backing method rather than
 * implementing a second gate that could drift from the first. This drives a
 * genuinely fenced daemon, not a stubbed error shape.
 */
externalTest("an MCP tool against a failed-closed daemon returns isError with the daemon's reason", async () => {
	const gateway = await hosted();

	// Fence the real daemon.
	gateway.core.setRpcHealth("failed_closed", "profile_drift");
	...
	const response = (await dispatchMcp(
		{ jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "registry_list", arguments: {} } },
		// biome-ignore lint/suspicious/noExplicitAny: the harness client satisfies the read surface.
		{ rpc: rpc as any, writeEnabled: false, serverVersion: "test" },
	)) as { result: { isError: boolean; content: Array<{ text: string }> } };

	expect(response.result.isError).toBe(true);
	// The daemon's own reason is surfaced, not a locally invented message.
	expect(response.result.content[0]?.text).toContain("profile_drift");
	expect(response.result.content[0]?.text).toContain("1000");

	// Observation tools the gateway still allows keep working while fenced.
	const status = (await dispatchMcp(
		{ jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "way_status", arguments: {} } },
		// biome-ignore lint/suspicious/noExplicitAny: the harness client satisfies the read surface.
		{ rpc: rpc as any, writeEnabled: false, serverVersion: "test" },
	)) as { result: { isError: boolean } };
