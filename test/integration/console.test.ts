...
function controlledTerminal(
	options: { readonly onWrite?: (text: string) => void | Promise<void> } = {},
): ControlledTerminal {
...
function recordedOutput(writer?: (text: string) => void | Promise<void>): {
...
function recordingClient(client: JsonRpcClient): {
...
function bareConsoleGateway(
	name: string,
	health: "running" | "failed_closed",
): {
...
externalTest(
	"console consumes the external supervisor UDS surface and restart resumes its checkpoint without duplicate render",
	async () => {
		...
		await eventually(
			() =>
				active.core.journalRead("1:0", 20).events.some((event) => event.kind === "assistant_message")
					? true
					: undefined,
...
externalTest("console commits a journal checkpoint only after a stalled terminal publication resolves", async () => {
	...
	const appended = active.core.journalAppend(
		"assistant_message",
...
externalTest(
	"terminal publication failure leaves the checkpoint unadvanced and fences further console submission",
	async () => {
		const active = await gateway();
		...
		expect(active.core.consumerCursor("gajaeway-console")).toBe(checkpointBefore);
		await expect(consoleSurface.submit("must remain fenced after failed publication")).rejects.toBeInstanceOf(
			ConsoleDeliveryUnavailableError,
		);
...
externalTest("healthy non-TTY console exits before claiming or consuming the journal checkpoint", async () => {
	...
	const pending = bare.core.journalAppend(
		"assistant_message",
		JSON.stringify({ finalized: true, text: "pending before TTY validation" }),
	);
	try {
		const ready = await connectEventually(path.join(bare.stateDirectory, "rpc.sock"));
		ready.close();
		...
		terminal.send("/quit");
		await runWayConsole({ stateDir: bare.stateDirectory, profilePath: bare.profilePath }, [], {
...
externalTest(
	"real console command loop admits owner input during a held external turn as steer",
	async () => {
		const active = await gateway();
		const terminal = controlledTerminal();
		const running = runWayConsole({ stateDir: active.stateDirectory, profilePath: active.profilePath }, [], {
			terminal: terminal.terminal,
			exitDrainMs: 1_000,
			statusPollMs: 25,
		});
		try {
			...
			await eventually(
				() => (active.host.turnState === "busy" ? true : undefined),
				"owner prompt did not enter the busy state",
			);
			...
			expect(
				active.fixture.commands().map((command) => ({ operation: command.operation, text: command.text })),
			).toEqual([
			...
			if (typeof heldPrompt?.opRef !== "string")
				throw new Error("held console prompt was not recorded with an operation reference");
...
externalTest(
	"console drains a submitted broker admission and renders delivered_as before close",
	async () => {
		const active = await gateway();
		const terminal = controlledTerminal();
		const running = runWayConsole({ stateDir: active.stateDirectory, profilePath: active.profilePath }, [], {
...
externalTest(
	"console reports outstanding admissions when its exit grace elapses",
	async () => {
		const active = await gateway();
		const terminal = controlledTerminal();
...
externalTest(
	"delivery loss tears the console down immediately rather than waiting for the input loop",
	async () => {
		const active = await gateway();
		let failAssistant = false;
		const terminal = controlledTerminal({
...
externalTest(
	"raw console visibly refuses a same-chunk oversized line and continues with later owner input",
	async () => {
		const active = await gateway();
		...
		try {
			await eventually(
				() => (latestTuiFrame(output.writes).includes("gajaeway> ") ? true : undefined),
				"raw console did not begin reading input",
			);
			...
			input.send(`${accepted}\n`);
			await eventually(
				() => (submissions.length === 1 ? true : undefined),
				"normal input after oversized refusal was not submitted",
			);
...
externalTest(
	"raw console coalesces mixed oversized and full-queue refusals without accepting dropped input",
	async () => {
		const active = await gateway();
		const input = new RawInputHarness();
		const output = new RawOutputHarness();
		...
		try {
			...
			await eventually(
				() => (submissions.length === inFlight.length ? true : undefined),
				"console did not saturate owner operations",
			);
			input.send(`${oversized}\n${queued.join("\n")}\n${dropped}\n`);
			await eventually(
				() => (output.writes.some((write) => write.includes("Input refused:")) ? true : undefined),
				"mixed refusal was not rendered",
			);
			const refusal = visibleTerminalText(
				output.writes.filter((write) => write.includes("Input refused:")).at(-1) as string,
			);
...
externalTest(
	"raw console bounds its queued lines and preserves FIFO after operation saturation",
	async () => {
		const active = await gateway();
		const input = new RawInputHarness();
		const output = new RawOutputHarness();
		...
		try {
			...
			await eventually(
				() => (submissions.length === initial.length ? true : undefined),
				"console did not saturate owner operations",
			);
			for (const line of queued) input.send(`${line}\n`);
			for (const line of rejected) input.send(`${line}\n`);
			await eventually(
				() => (output.writes.join("").includes("Input queue is full") ? true : undefined),
				"full input queue was not visible",
			);
			expect(terminal.queuedLineCount).toBe(MAX_RAW_CONSOLE_QUEUED_LINES);
			expect(terminal.queuedInputBytes).toBeLessThanOrEqual(MAX_RAW_CONSOLE_QUEUED_BYTES);
...
externalTest(
	"raw Ctrl-C stays observable at saturated queue capacity and starts bounded drain",
	async () => {
...
externalTest(
	"raw Ctrl-D stays observable at saturated queue capacity and starts bounded drain",
	async () => {
		await assertRawSaturatedControlStartsDrain("\u0004");
	},
	15_000,
...
externalTest(
	"full-screen prompt repaint preserves assistant and gate frames while editing",
	async () => {
		const active = await gateway();
		const input = new RawInputHarness();
		const output = new RawOutputHarness();
		...
		try {
			await eventually(
				() => (latestTuiFrame(output.writes).includes("gajaeway> ") ? true : undefined),
				"console did not leave an active editor",
			);
			input.send("draft while events arrive");
			await eventually(
				() => (latestTuiFrame(output.writes).includes("gajaeway> draft while events arrive") ? true : undefined),
				"editor did not repaint draft",
			);
			active.core.journalAppend(
				"assistant_message",
				JSON.stringify({ finalized: true, text: "assistant frame remains visible" }),
			);
			const gate = active.core.journalAppend(
				"gate_open",
...
externalTest(
	"cockpit status pane renders lock quarantine and reconcile state from way.status",
	async () => {
		const active = await gateway();
		...
		try {
			...
			await eventually(
				() =>
					terminal.writes.some((write) => write.startsWith("Gateway status") && write.includes("quarantined=true"))
						? true
						: undefined,
...
externalTest(
	"cockpit renders durable-tail loss notices as warnings and exposes them in journal filters",
	async () => {
		const active = await gateway();
		...
		try {
			...
			await eventually(
				() =>
					terminal.writes.some((write) => write.includes("WARNING: Lifecycle event-ring retention advanced"))
						? true
						: undefined,
			...
			await eventually(
				() =>
					terminal.writes.some((write) => write.includes("WARNING: Transcript delivery gap detected"))
						? true
						: undefined,
			...
			await eventually(
				() =>
					terminal.writes.some(
						(write) => write.startsWith("Journal tail:") && write.includes("transcript_delivery_gap"),
					)
						? true
						: undefined,
			...
			const filteredRead = recorded.calls
				.filter(
					(call) =>
						call.method === "main.events.read" &&
						Array.isArray((call.params as Record<string, unknown> | undefined)?.kinds),
				)
				.at(-1);
...
externalTest(
	"cockpit /journal filters registry noise by default and honors an explicit kind",
	async () => {
		const active = await gateway();
		const recorded = recordingClient(active.client);
		const terminal = controlledTerminal();
		...
		try {
			...
			const defaultRead = recorded.calls
				.filter(
					(call) =>
						call.method === "main.events.read" &&
						typeof (call.params as Record<string, unknown> | undefined)?.cursor === "string",
				)
				.at(-1);
			...
			await eventually(
				() =>
					terminal.writes
						.filter((write) => write.startsWith("Journal tail:"))
						.some((write) => write.includes("registry_change"))
						? true
						: undefined,
...
externalTest(
	"cockpit /registry list and inspect render durable UDS registry rows",
	async () => {
		const active = await gateway();
...
externalTest(
	"cockpit /lock force-release requires exact confirmation and renders the gateway refusal",
	async () => {
		const active = await gateway();
		...
		const running = runWayConsole({ stateDir: active.stateDirectory, profilePath: active.profilePath }, [], {
			terminal: terminal.terminal,
			rpcConnect: async () => recorded.rpc,
			idempotencyKey: () => "cockpit-force-release",
			statusPollMs: 25,
		});
		try {
			await eventually(() => (terminal.isReading() ? true : undefined), "cockpit did not begin reading input");
			...
			expect(calls[0]?.params).toEqual({
				...
				confirm: true,
				idempotency_key: "cockpit-force-release",
			});
			terminal.send("/quit");
			await running;
		} finally {
...
externalTest(
	"console command loop renders the external-host gate limitation and help",
	async () => {
		const active = await gateway();
		const terminal = controlledTerminal();
...
externalTest(
	"cockpit /schedule renders durable scheduler jobs and run history read-only",
	async () => {
		const active = await gateway();
		active.core.scheduleJobUpsert({
			jobId: "cockpit-job-1",
			name: "cockpit nudge",
			kind: "every",
			spec: String(60_000),
			...
			payloadJson: JSON.stringify({ text: "cockpit scheduled nudge" }),
		...
		try {
			await eventually(() => (terminal.isReading() ? true : undefined), "cockpit did not begin reading input");
			terminal.send("/schedule list");
			await eventually(
				() => (terminal.writes.some((write) => write.startsWith("Schedule list:")) ? true : undefined),
				"schedule list did not render",
			);
			const listed = terminal.writes.filter((write) => write.startsWith("Schedule list:")).at(-1) as string;
			expect(listed).toContain("cockpit-job-1");
			expect(listed).toContain("state=active");
			expect(listed).toContain("payload=system_event");

			terminal.send("/schedule runs cockpit-job-1");
			await eventually(
				() => (terminal.writes.some((write) => write.startsWith("Schedule runs for cockpit-job-1")) ? true : undefined),
				"schedule runs did not render",
			);
			const runs = terminal.writes
				.filter((write) => write.startsWith("Schedule runs for cockpit-job-1"))
				.at(-1) as string;
			expect(runs).toContain("shown=0");

			// An unknown job surfaces the gateway refusal rather than an empty list.
			terminal.send("/schedule runs missing-job");
			await eventually(
				() => (terminal.writes.some((write) => write.startsWith("Request failed:")) ? true : undefined),
				"unknown schedule job did not surface a refusal",
			);

			// Usage guidance rather than a silent no-op for a malformed subcommand.
			terminal.send("/schedule bogus");
			await eventually(
				() => (terminal.writes.some((write) => write.includes("Usage: /schedule list")) ? true : undefined),
				"malformed schedule command did not render usage",
			);
			terminal.send("/quit");
			await running;
		} finally {
